...
renach |> 
  ...
  summarise(.by = c(ano, Sexo, Categoria), condutores = sum(condutores)) |> 
...
heat_prf_geral_fem <- 
  ...
  ggplot(aes(estado_fisico, tipo_acidente, fill = n)) +
    geom_tile() +
    geom_text(aes(label = n, 
                  color = ifelse(n > max(n)/2, "grey90", "black")), 
              size = 3) +
    scale_fill_gradient(high = onsv_palette$blue, low = "#a2c6d8") +
...
heat_prf_geral_masc <- 
  ...
  ggplot(aes(estado_fisico, tipo_acidente, fill = n)) +
    ...
    geom_text(aes(label = n),
              color = "black",
              size = 3) +
    scale_fill_gradient(high = onsv_palette$yellow, low = "#f7dbb9") +
    scale_color_identity() +
    theme_minimal() +
    labs(x = NULL, y = NULL) +
    theme(legend.position = "none")
    
ggplotly(heat_prf_geral_fem, tooltip = "text")
ggplotly(heat_prf_geral_masc, tooltip = "text")
```
...
prf_condutores_2023 |> 
  ...
  mutate(Total = Ferido + Ileso + `Óbito`,
         ObsPorTotal = `Óbito`/Total * 100) |> 
  gt()
```
